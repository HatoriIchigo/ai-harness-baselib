...
トリガ宣言機構は持たない。`ai-harness-main` は hook 発火ごとに**全プラグインを並列起動**する。各プラグインは `Action` 内で `HookData` を見て**自己フィルタ**し、対象外なら `yield break`（`ExitCode` は 0 のまま＝許可）。
...
- ターゲット: net8.0 / nullable 有効 / 暗黙 usings 有効
